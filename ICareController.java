...
import org.openmrs.module.icare.core.models.SMSAction;
...
//		}
	...
	public ResponseEntity<String> handleRequest(@RequestBody SMSAction smsAction) throws JSONException {
		...
		if (ICareConfig.ACTION_INCOMING.equals(action)){
			...
			if (ICareConfig.MESSAGE_TYPE_SMS.equals(messageType)){
				String jsonResponse = iCareService.processIncomingAction(from,message,messageType);
			...
			else if (ICareConfig.MESSAGE_TYPE_MMS.equals(messageType)) {
				String jsonResponse = iCareService.processIncomingAction(from,message,messageType);
			...
			else if (ICareConfig.MESSAGE_TYPE_CALL.equals(messageType)) {
				String jsonResponse = iCareService.processIncomingCall(from,message,messageType);
		...
		else if (ICareConfig.ACTION_SEND_STATUS.equals(action)) {
			String ID = smsAction.getId();
			String error_message = smsAction.getMessage();
			String status = smsAction.getStatus();
			if (ICareConfig.STATUS_FAILED.equals(status)){
				String notification = iCareService.errorResponse(error_message);
			...
			else if (ICareConfig.STATUS_SENT.equals(status)) {

				String notification = iCareService.sentResponse(ID,status);
				...
				String response = iCareService.queuedResponse(ID,status);
